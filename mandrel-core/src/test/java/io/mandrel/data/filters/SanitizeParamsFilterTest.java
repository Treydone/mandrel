...
public class SanitizeParamsFilterTest {
	...
	public void link_without_params() {
		Link link = new Link().setUri(Uri.create("http://localhost/1"));
		Assertions.assertThat(filter.isValid(link)).isTrue();
		Assertions.assertThat(link.getUri()).isEqualTo(Uri.create("http://localhost/1"));
	}
	...
	public void link_with_param_empty() {
		Link link = new Link().setUri(Uri.create("http://localhost/1?test"));
		Assertions.assertThat(filter.isValid(link)).isTrue();
		Assertions.assertThat(link.getUri()).isEqualTo(Uri.create("http://localhost/1"));
	}
	...
	public void link_with_param_empty_keept() {
		...
		Assertions.assertThat(filter.isValid(link)).isTrue();
		Assertions.assertThat(link.getUri()).isEqualTo(Uri.create("http://localhost/1?test"));
	}

	@Test
	public void link_with_params_all_filtered() {
		Link link = new Link().setUri(Uri.create("http://localhost/1?foo=test&foo2=test2"));
		Assertions.assertThat(filter.isValid(link)).isTrue();
		Assertions.assertThat(link.getUri()).isEqualTo(Uri.create("http://localhost/1"));
	}
	...
	public void link_with_params_and_keep_one() {
		...
		Assertions.assertThat(filter.isValid(link)).isTrue();
		Assertions.assertThat(link.getUri()).isEqualTo(Uri.create("http://localhost/1?foo2=test2"));
	}

	@Test
	public void link_with_params_and_keep_mutliple() {
		Link link = new Link().setUri(Uri.create("http://localhost/1?foo=test&foo2=test2&foo2=test3&z=13"));
		filter.setExclusions(Arrays.asList("foo2", "foo"));
		Assertions.assertThat(filter.isValid(link)).isTrue();
		Assertions.assertThat(link.getUri()).isEqualTo(Uri.create("http://localhost/1?foo=test&foo2=test2&foo2=test3"));
	}
