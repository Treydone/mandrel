...
public class Realm {
	...
	public boolean equals(Object o) {
		...
		if (uri != null ? !uri.equals(realm.uri) : realm.uri != null)
			return false;
		if (useAbsoluteURI != !realm.useAbsoluteURI)
			return false;
		if (omitQuery != !realm.omitQuery)
			return false;
	...
	public static class RealmBuilder {
		...
		//
		// Portions of code (newCnonce, newResponse) are highly inspired be
		// Jetty 6 BasicAuthentication.java class.
		...
		public RealmBuilder clone(Realm clone) {
			return setRealmName(clone.getRealmName())//
					...
					.setUri(clone.getUri())//
					.setUsePreemptiveAuth(clone.isUsePreemptiveAuth())//
		...
		public Realm build() {
			...
			return new Realm(scheme, principal, password, realmName, nonce, algorithm, response, qop, nc, cnonce, uri, methodName, usePreemptive, ntlmDomain,
					charset, ntlmHost, opaque, useAbsoluteURI, omitQuery, targetProxy);
		}
	}
}
