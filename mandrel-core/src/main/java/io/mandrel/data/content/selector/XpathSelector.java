...
public class XpathSelector extends BodySelector<XElement> {
	...
	public Instance<XElement> init(BlobMetadata data, Payload payload, boolean isSegment) {
		...
		try {
			if (!isSegment) {
				element = Jsoup.parse(payload.openStream(), Charsets.UTF_8.name(), data.getUri().toString());
			} else {
				element = Jsoup.parseBodyFragment(IOUtils.toString(payload.openStream()), data.getUri().toString()).body();
			}
		} catch (IOException e) {
			payload.release();
