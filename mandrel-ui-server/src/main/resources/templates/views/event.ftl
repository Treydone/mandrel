									...
									<#if (event.node??) && (event.node.type??)>
										<#switch event.node.type>
											<#case "NODE_STARTED">
												...
												<#assign footer = '<a class="btn btn-info btn-flat btn-xs" href="/nodes/${event.node.nodeId}">View node</a>'>
												<#break>
											<#case "NODE_STOPPED">
												<#assign icon = "fa-laptop">
												<#assign color = "bg-red">
									...
									<#if (event.job??) && (event.job.type??)>
										<#switch event.job.type>
											<#case "SPIDER_CREATED">
												...
												<#assign footer = '<a class="btn btn-info btn-flat btn-xs" "/jobs/${event.job.jobId}">View job</a>'>
					 							<#break>
					 						<#case "SPIDER_STARTED">
					 							<#assign icon = "fa-tasks">
