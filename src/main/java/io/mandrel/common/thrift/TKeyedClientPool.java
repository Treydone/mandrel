...
@Data
public class TKeyedClientPool<T extends TServiceClient> implements AutoCloseable {

	private static final Logger LOGGER = LoggerFactory.getLogger(TKeyedClientPool.class);
	...
	public TKeyedClientPool(ClientFactory<T> clientFactory, GenericKeyedObjectPoolConfig poolConfig) {
		this(clientFactory, poolConfig, DEFAULT_PORT, DEFAULT_DEFLATE_LEVEL);
	}

	public TKeyedClientPool(ClientFactory<T> clientFactory, GenericKeyedObjectPoolConfig poolConfig, int defaultPort, Integer deflateLevel) {
		this(clientFactory, new BinaryOverSocketProtocolFactory(deflateLevel, deflateLevel), poolConfig);
	}

	public TKeyedClientPool(ClientFactory<T> clientFactory, ProtocolFactory protocolFactory, GenericKeyedObjectPoolConfig poolConfig) {
		this.protocolFactory = protocolFactory;
		this.internalPool = new GenericKeyedObjectPool<HostAndPort, T>(new ThriftClientFactory(clientFactory, protocolFactory), poolConfig);
	}
	...
	class ThriftClientFactory extends BaseKeyedPooledObjectFactory<HostAndPort, T> {
