...
import io.mandrel.common.robots.ExtendedRobotRules.RobotRulesMode;
...
public class ExtendedRobotRulesParser extends BaseRobotsParser {
	private static final Logger LOGGER = LoggerFactory.getLogger(ExtendedRobotRulesParser.class);
	...
	private static class ParseState {
		...
		private ExtendedRobotRules _curRules;

		public ParseState(String url, String targetName) {
			_url = url;
			_targetName = targetName;
			_curRules = new ExtendedRobotRules();
		}
		...
		public ExtendedRobotRules getRobotRules() {
			return _curRules;
		}
	...
	public BaseRobotRules parseContent(String url, byte[] content, String contentType, String robotName) {
		...
		if ((content == null) || (content.length == 0)) {
			return new ExtendedRobotRules(RobotRulesMode.ALLOW_ALL);
		}
		...
		ParseState parseState = new ParseState(url, robotName.toLowerCase(Locale.ROOT));
		...
		ExtendedRobotRules result = parseState.getRobotRules();
		if (result.getCrawlDelay() > MAX_CRAWL_DELAY) {
			// Some evil sites use a value like 3600 (seconds) for the crawl
			// delay, which would
			// cause lots of problems for us.
			LOGGER.debug("Crawl delay exceeds max value - so disallowing all URLs: " + url);
			return new ExtendedRobotRules(RobotRulesMode.ALLOW_NONE);
		} else {
			result.sortRules();
			return result;
		}
